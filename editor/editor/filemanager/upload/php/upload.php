...
if(!is_dir($sServerDir.date(Ym)))

mkdir($sServerDir.date(Ym),0755); 
...
while ( true )
{
	...
	else
	{
		...
		if ( $Config['UseFileType'] )
			$sFileUrl = $Config["UserFilesPath"] . $sType . '/'.date(Ym).'/' . $sFileName ;
		else
			$sFileUrl = $Config["UserFilesPath"] .date(Ym).'/'. $sFileName ;
